            ...
            <div class="col-12">
                ...
                <div class="admin-sec input-w-unset mt-85">
                    <h3 class="admin-sec-title">
                        Edit {{$build->name}} Property
                    </h3>
                    <form action="{{route('developer.project.update' , $build->id)}}" method="post"
                                ...
                                <div class=" flex-auto ms-4 vertical-top border-left-custom developer-general-info">
                                    <div class="profile-input-custom d-flex">
                                        ...
                                        <div class="input-custom flex-auto self no-logo">
                                            ...
                                            <input type="text" required name="name"
                                                   placeholder="Type Your Project Name" value="{{$build->name}}">
                                        </div>
                                    ...
                                    <div id="deposit-sec-maker">
                                        @foreach($build->deposits as $deposit)
                                            <div class="deposit-cart"><p>{{$deposit->number}}%</p>
                                                <p>{{$deposit->text}}<input type="hidden" name="depositNumber[]"
                                                                            value="{{$deposit->number}}"><input
                                                        type="hidden" name="depositText[]" value="{{$deposit->text}}">
                                                </p><span
                                                    class="delete-deposit-btn" onclick="deleteDeposit(this)"> <svg
                                                        ...
                                                        xmlns="http://www.w3.org/2000/svg"> <path
                                                            ...
                                                            fill="#4A5568"> &lt; /path&gt;</path></svg></span></div>


                                            <div class="deposit-cart-seprator"><img
                                                    src="http://127.0.0.1:8000/img/Line 67.svg" alt=""></div>
                                    ...
                                    <div class="profile-input-custom ">
                                        ...
                                        <div id="assi-no"
                                             class="input-custom Assignment-no @if($build->assignment == 'No')  assignment-active @endif flex-auto self no-logo"
                                             onclick="AssignmentNo()">

                                            <input type="text" readonly placeholder="No" value="No"
                                                   @if($build->assignment == 'No')  name="assignment" @endif>
                                        </div>
                                        <div id="assi-yes"
                                             class="input-custom Assignment-no-secto @if($build->assignment !== 'No')  assignment-active @endif  self no-logo"
                                             onclick="AssignmentYes()">

                                            <input type="text" placeholder="If Yes, Type Here..."
                                                   @if($build->assignment !== 'No')  name="assignment"
                                                   value="{{$build->assignment}}" @endif >
                                        </div>
                                    </div>
                                    ...
                                    <div class="phase-section">
                                        @foreach($build->phases as $phase)
                                            <p class="green-gray-raduis ml-12 position-relative d-inline-block">
                                                {{$phase->type}},{{$phase->name}}<input
                                                    type="hidden" name="phasesType[]" value="{{$phase->type}}"><input
                                                    type="hidden"
                                                    name="phasesName[]"
                                                    value="{{$phase->name}}"><span
                                                    class="delete-pase-btn" onclick="deletePhase(this)"> <svg width="15"
                                                                                                              height="15"
                                                                                                              viewBox="0 0 22 22"
                                                                                                              fill="none"
                                                                                                              xmlns="http://www.w3.org/2000/svg"><path
                                                            d="M10.7489 12.1037L6.15203 16.7006L4.91531 15.4639L9.51221 10.867L4.91531 6.27008L6.1287 5.05668L10.7256 9.65358L15.3225 5.05668L16.5592 6.29341L11.9623 10.8903L16.5592 15.4872L15.3458 16.7006L10.7489 12.1037Z"
                                                            fill="#4A5568"></path></svg></span></p>
...
@section('custom-script')
    ...
    <script>
        ...
        var coverFile = [
            {
                ...
                options: {
                    ...
                    file: {
                        name:
                            '{{$build->cover_image->name}}',
                    },

                    // pass poster property
                    metadata: {
                        poster:
                            '{{asset('storage/'.($build->cover_image->slug))}}',
                    },
                },
            },]
        @else
        ...
        var imagesFile = [
                @foreach($build->image_gallery as $image)
            {
        ...
        var promotionFile = [
            {
                ...
                options: {
                    ...
                    file: {
                        name:
                            '{{$build->promotion_image?->name}}',
                    },

                    // pass poster property
                    metadata: {
                        poster:
                            '{{asset('storage/'.($build->promotion_image?->slug))}}',
                    },
                },
            },]
        @else
